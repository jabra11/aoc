...
for y in range(ylen):
    for x in range(xlen):
        if scaled[y][x] == '.':
            if (is_connected(scaled, (y,x-1), (y,x+1))):
                scaled[y][x] = '-'
            elif (is_connected(scaled, (y-1,x), (y+1,x))):
                scaled[y][x] = '|'
...
floodfill(scaled,0,0)
floodfill(scaled,len(scaled)-1,0)
floodfill(scaled,0,len(scaled[0])-1)
floodfill(scaled,len(scaled)-1,len(scaled[0])-1)
...
for y in range(ylen):
    for x in range(xlen):
        sb = scaleback(y,x)
        if scaled[y][x] == '.' and sb not in loop:
            if sb not in vis:
                answ += 1
                vis.append(sb)
print(answ)
